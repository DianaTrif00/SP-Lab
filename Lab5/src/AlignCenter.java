...
public class AlignCenter implements AlignStrategy {
    public String center(String line, int nr_char_in_line){
        int paddingsize = nr_char_in_line - line.length();
        int paddingstart = line.length() + paddingsize/2;
        line = String.format("%" + paddingstart + "s" , line);
        line = String.format("%-" + nr_char_in_line + "s" , line);
        return line;
    }
    ...
    public String render(Paragraph paragraph, Context context){
        ...
        for (String word: text_splited){
            if (nr_char_in_line + word.length() < number_char_on_line){
                ...
                new_text.add(center(String.join(" ",line) + "\n", number_char_on_line));
                line = new ArrayList<>();
                line.add(word);
        ...
        if (line.size() > 0){
            new_text.add(center(String.join(" ",line), number_char_on_line));
        }
